...
filenames = {['MLE_' inits '35.mat'], ['MLE_' inits '65.mat']};
trials = [];
for ff=1:2
    load(filenames{ff});
    trials = [trials; MLE];
end
trials(:,11) = [35*ones(156,1);65*ones(156,1)];


% find the unimodal trials
vision_ind = trials(:,5)==1 & trials(:,6)==0;
vision_tmp = (trials(vision_ind,:));
vision = table(vision_tmp(:,1),vision_tmp(:,2),vision_tmp(:,11),'VariableNames',{'level','choice','noise'});
unimodal = {};
g = grpstats(vision(vision.noise==35,:),{'level'},{'sum','mean'});
unimodal{1} = [g.level,g.sum_choice,g.GroupCount];
g = grpstats(vision(vision.noise==65,:),{'level'},{'sum','mean'});
...
unimodal{3} = [g.level,g.sum_choice,g.GroupCount];
...
conditions = {'vision(35)', 'vision(65)', 'proprioception'};
figure(1); clf;
for ii=1:3
...
subplot(224);
suggested_noise_level = ceil(interp1(sig(1:2),[35,65],sig(3)));
fprintf('suggested noise level: %i\n',suggested_noise_level)
for ii=1:3
...
xlim([.5,2.5]);
r=refline(0,sig(3));
set(r,'LineWidth',2,'LineStyle','--');
set(gca','XTick',[1,2],'XTickLabels',{35,65});
xlabel('visual noise');
ylabel('sigma');
